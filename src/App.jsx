...
import {
  ...
  Leaf, ClipboardList, Phone, Ruler, Clock, Crown, ShieldCheck,
  LocateFixed, IndianRupee, Layers, ArrowRight, Lock, LayoutGrid
} from "lucide-react";

/* ============================================================
   GreenGrow — Agro Agency Demo Management (prototype, fake data)
   Model: a demo = one (farmer × crop) with its own stage track.
   A farmer can run MANY demos (different crops, different stages).
   ============================================================ */
...
const SEED_CROPS = [
  { id: "c1", name: "Cotton",    icon: "🌱", season: "Kharif",
    stages: ["Sowing", "Germination", "Vegetative", "Squaring", "Flowering", "Boll Formation", "Harvest"] },
  { id: "c2", name: "Groundnut", icon: "🥜", season: "Kharif",
    stages: ["Sowing", "Germination", "Vegetative", "Pegging", "Pod Development", "Maturity", "Harvest"] },
  { id: "c3", name: "Wheat",     icon: "🌾", season: "Rabi",
    stages: ["Sowing", "Germination", "Tillering", "Jointing", "Heading", "Grain Filling", "Harvest"] },
];
...
/* one farmer (f1 Bharat) intentionally runs 3 crops at 3 different stages */
const SEED_DEMOS = [
  { id: "d1", cropId: "c1", farmerId: "f1", assignedTo: "u_emp1", productIds: ["p1", "p7"], start: "2026-07-02", stageIndex: 4 },
  { id: "d6", cropId: "c2", farmerId: "f1", assignedTo: "u_emp1", productIds: ["p2", "p6"], start: "2026-07-20", stageIndex: 3 },
  { id: "d7", cropId: "c3", farmerId: "f1", assignedTo: "u_emp1", productIds: ["p3", "p4"], start: "2026-08-05", stageIndex: 1 },
  { id: "d2", cropId: "c2", farmerId: "f2", assignedTo: "u_emp1", productIds: ["p2", "p6"], start: "2026-07-10", stageIndex: 2 },
  { id: "d3", cropId: "c1", farmerId: "f3", assignedTo: "u_emp2", productIds: ["p1", "p8"], start: "2026-06-28", stageIndex: 6 },
  { id: "d8", cropId: "c2", farmerId: "f3", assignedTo: "u_emp2", productIds: ["p2", "p5"], start: "2026-07-18", stageIndex: 2 },
  { id: "d4", cropId: "c2", farmerId: "f4", assignedTo: "u_emp3", productIds: ["p2", "p5"], start: "2026-07-15", stageIndex: 1 },
  { id: "d5", cropId: "c3", farmerId: "f5", assignedTo: "u_emp4", productIds: ["p3", "p4"], start: "2026-08-01", stageIndex: 0 },
];
...
const CAPS = {
  admin:    { manageProducts: true,  manageCrops: true,  manageTeam: true,  viewTeam: true,  scope: "all"  },
  manager:  { manageProducts: false, manageCrops: false, manageTeam: false, viewTeam: true,  scope: "team" },
  employee: { manageProducts: false, manageCrops: false, manageTeam: false, viewTeam: false, scope: "own"  },
};
...
export default function App() {
  ...
  const teamEmployeeIds = useMemo(
    () => PEOPLE.filter((p) => p.role === "employee" && p.manager === me.id).map((p) => p.id),
    [me.id]
  );
  const inScope = (ownerId) =>
    caps.scope === "all" ? true : caps.scope === "team" ? teamEmployeeIds.includes(ownerId) : ownerId === me.id;

  const visFarmers = farmers.filter((f) => inScope(f.addedBy));
  const visDemos = demos.filter((d) => inScope(d.assignedTo));
  const visMeetings = meetings;

  const flash = (msg) => { setToast(msg); setTimeout(() => setToast(null), 2600); };

  const resetViews = () => { setOpenDemo(null); setOpenCrop(null); setOpenFarmer(null); };
  ...
  const addStage = (cropId, stage) => {
    setCrops((s) => s.map((c) => (c.id === cropId ? { ...c, stages: [...c.stages, stage] } : c)));
    flash("Stage added to pipeline");
  };

  /* standalone add (Farmers page) — blocks duplicates */
  const addFarmerStandalone = (f) => {
    ...
    setFarmers((s) => [{ ...f, id: uid(), addedBy: me.id }, ...s]);
    flash("Farmer captured with live location");
    return true;
  };
  /* field flow — returns an id, reusing an existing farmer if matched */
  const resolveFarmerId = (f) => {
    const dupe = farmers.find((x) => sameFarmer(x, f));
    if (dupe) { flash(`${dupe.name} already on record — using existing farmer`); return dupe.id; }
    const id = uid();
    setFarmers((s) => [{ ...f, id, addedBy: me.id }, ...s]);
    return id;
  };

  const createDemo = ({ cropId, farmerId, productIds, start }) =>
    setDemos((s) => [{ id: uid(), cropId, farmerId, productIds, start, stageIndex: 0, assignedTo: me.id }, ...s]);

  const submitDemo = (d) => {
    const farmerId = d.mode === "new" ? resolveFarmerId(d.farmer) : d.farmerId;
    createDemo({ cropId: d.cropId, farmerId, productIds: d.productIds, start: d.start });
    flash("Crop demo started");
    setModal(null);
  };

  const advanceStage = (demoId) =>
    setDemos((s) => s.map((d) => {
      if (d.id !== demoId) return d;
      const max = crops.find((c) => c.id === d.cropId).stages.length - 1;
      return { ...d, stageIndex: Math.min(d.stageIndex + 1, max) };
    }));
  ...
  return (
    <div className="gg">
        ...
        <nav className="nav">
          {NAV.map((n) => (
            <button key={n.id} className={"nav-item" + (page === n.id ? " active" : "")} onClick={() => go(n.id)}>
              ...
              {page === n.id && <ChevronRight size={16} className="nav-caret" />}
            </button>
          ))}
        </nav>
      ...
      <div className="main">
        <header className="topbar">
          ...
          <div className="page-head">
            <h1>{NAV.find((n) => n.id === page)?.label || "Dashboard"}</h1>
            <p>{me.name} · <span className="role-pill">{role}</span></p>
          </div>
        ...
        <main className="content">
          {openDemo
            ? <DemoDetail demo={openDemo} crops={crops} farmers={farmers} products={products} onBack={() => setOpenDemo(null)} onAdvance={advanceStage} />
            : openFarmer
            ? <FarmerDetail farmer={openFarmer} demos={demos} crops={crops} me={me} onBack={() => setOpenFarmer(null)} onOpenDemo={setOpenDemo} onStart={startFor} onAdvance={advanceStage} />
            : openCrop
            ? <CropDetail crop={crops.find((c) => c.id === openCrop.id) || openCrop} demos={demos} caps={caps} onBack={() => setOpenCrop(null)} onAddStage={(s) => addStage(openCrop.id, s)} />
            : (() => {
                switch (page) {
                  case "dashboard": return <Dashboard {...{ me, caps, visDemos, visFarmers, visMeetings, crops, setModal, setOpenDemo, go }} />;
                  case "products":  return <Products {...{ products, caps, setModal }} />;
                  case "crops":     return <Crops {...{ crops, caps, demos, setOpenCrop, setModal }} />;
                  case "demos":     return <Demos {...{ visDemos, crops, farmers, setModal, setOpenDemo }} />;
                  case "track":     return <Track {...{ visFarmers, visDemos, crops, caps, onOpenDemo: setOpenDemo, onOpenFarmer: setOpenFarmer, onStart: startFor }} />;
      ...
      {modal?.type === "demo"    && <DemoModal onClose={() => setModal(null)} onSave={submitDemo} {...{ crops, products, visFarmers, presetFarmerId: modal.presetFarmerId, presetCropId: modal.presetCropId }} />}
...
const sameFarmer = (a, b) =>
  a.phone.replace(/\s/g, "") === b.phone.replace(/\s/g, "") || a.name.trim().toLowerCase() === b.name.trim().toLowerCase();
...
function RoleSwitch({ role, onChange }) {
  ...
  return (
    <div className="roleswitch">
      {opts.map((o) => (
        <button key={o.id} className={"rs-opt" + (role === o.id ? " on" : "")} onClick={() => onChange(o.id)}>
          <o.icon size={14} /> <span>{o.label}</span>
        </button>
      ))}
    </div>
  );
          ...
          return (
            <div className={"node " + state} key={i} style={{ left: `${(i / (stages.length - 1)) * 100}%` }}>
              <span className="node-dot">
                {state === "done" && <Check size={compact ? 10 : 13} />}
                {state === "current" && <Sprout size={compact ? 11 : 14} />}
              </span>
              {!compact && <span className="node-label">{s}</span>}
            </div>
          );
        })}
      </div>
...
function MiniDots({ stages, index }) {
  return (
    <div className="dots">
      {stages.map((_, i) => <span key={i} className={"dot-mini " + (i < index ? "done" : i === index ? "cur" : "todo")} />)}
    </div>
  );
}

/* ============================================================ Dashboard */
function Dashboard({ me, caps, visDemos, visFarmers, visMeetings, crops, setModal, setOpenDemo, go }) {
  const active = visDemos.filter((d) => d.stageIndex < crops.find((c) => c.id === d.cropId).stages.length - 1);
    ...
    <div className="stack">
      <div className="hero">
        <div>
          ...
          <p className="hero-sub">Every demo is one farmer × one crop, tracked stage by stage.</p>
        </div>
      ...
      <div className="kpi-grid">
        ...
        <Stat icon={Layers} tone="amber" label="Stages advanced" value={stagesAdvanced} sub="this season" />
      ...
      <div className="two-col">
        <section className="card">
          ...
          <div className="stack-sm">
            {active.slice(0, 4).map((d) => {
              ...
              return (
                <button className="demo-row" key={d.id} onClick={() => setOpenDemo(d)}>
                  <span className="crop-chip">{c.icon}</span>
                  <div className="demo-row-main">
                    <div className="demo-row-top"><strong>{c.name} demo</strong><span className="badge badge-green">{c.stages[d.stageIndex]}</span></div>
                    <GrowthTrack stages={c.stages} index={d.stageIndex} compact />
                  </div>
        ...
        <section className="card">
          ...
          <div className="bars">
            {byCrop.map((b) => (
              <div className="bar-row" key={b.name}>
                <span className="bar-label">{b.icon} {b.name}</span>
                <div className="bar-track"><div className="bar-fill" style={{ width: `${(b.count / maxCount) * 100}%` }} /></div>
          ...
          <div className="stack-xs">
            {upcoming.map((m) => (
              <div className="mini-meet" key={m.id}>
                <div className="date-chip"><span>{fmtDate(m.date).split(" ")[0]}</span><em>{fmtDate(m.date).split(" ")[1]}</em></div>
                <div><div className="mm-title">{m.title}</div><div className="mm-sub"><Clock size={12} /> {m.time} · {byId(m.host).name}</div></div>
              </div>
            ))}
          </div>
        </section>
...
function Stat({ icon: Icon, label, value, sub, tone }) {
  return (
    <div className={"kpi kpi-" + tone}>
      <span className="kpi-ic"><Icon size={18} /></span>
      ...
      <div className="prod-grid">
        {list.map((p) => (
          <div className="prod-card" key={p.id}>
            ...
            <div className="prod-name">{p.name}</div><div className="prod-brand">{p.brand}</div>
            <div className="prod-foot"><Package size={13} /> {p.pack}</div>
          </div>
        ))}
      </div>
    </div>
...
function Crops({ crops, caps, demos, setOpenCrop, setModal }) {
  return (
    <div className="stack">
      <SectionHead title="Crops & growth stages" sub="Each crop carries the stage pipeline a demo moves through — sowing to harvest"
        action={caps.manageCrops && <button className="btn btn-primary" onClick={() => setModal({ type: "crop" })}><Plus size={16} /> New crop</button>}
        note={caps.manageCrops ? <span className="ro-note ok"><ShieldCheck size={13} /> You own crops & stages — add either</span>
                               : <span className="ro-note"><Lock size={13} /> Crops & stages are defined by the owner</span>} />
      <div className="crop-grid">
        {crops.map((c) => {
          const running = demos.filter((d) => d.cropId === c.id).length;
          return (
            <button className="crop-card" key={c.id} onClick={() => setOpenCrop(c)}>
              <div className="crop-card-head">
                ...
                <span className="stage-count">{c.stages.length} stages</span>
              </div>
              <GrowthTrack stages={c.stages} index={c.stages.length - 1} compact />
              <div className="crop-stage-tags">
                {c.stages.slice(0, 4).map((s) => <span key={s} className="tag">{s}</span>)}
                {c.stages.length > 4 && <span className="tag more">+{c.stages.length - 4}</span>}
              </div>
...
function CropDetail({ crop, demos, caps, onBack, onAddStage }) {
  ...
  return (
    <div className="stack">
      ...
      <div className="detail-hero">
        <span className="crop-emoji xl">{crop.icon}</span>
        <div><h2 className="detail-title">{crop.name}</h2><p className="detail-sub">{crop.season} season · {crop.stages.length} stages · {running} demo{running !== 1 && "s"} running</p></div>
      </div>
      <div className="card">
        <div className="card-head"><h3>Stage pipeline</h3></div>
        <div className="pipeline">
          {crop.stages.map((s, i) => (
            <div className="pipe-step" key={i}>
              <span className="pipe-num">{String(i + 1).padStart(2, "0")}</span><span className="pipe-name">{s}</span>
              {i < crop.stages.length - 1 && <span className="pipe-arrow"><ArrowRight size={14} /></span>}
            </div>
          ))}
        </div>
        {caps.manageCrops && (
          <div className="add-stage">
            <input placeholder="New stage name (e.g. Top Dressing)" value={val} onChange={(e) => setVal(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter" && val.trim()) { onAddStage(val.trim()); setVal(""); } }} />
            <button className="btn btn-primary" disabled={!val.trim()} onClick={() => { onAddStage(val.trim()); setVal(""); }}><Plus size={16} /> Add stage</button>
          </div>
        )}
      </div>
    </div>
...
/* ============================================================ Demos */
function Demos({ visDemos, crops, farmers, setModal, setOpenDemo }) {
  return (
    <div className="stack">
      <SectionHead title="Crop demos" sub="Each card is one farmer × one crop. The same farmer can appear under several crops."
      ...
      <div className="demo-grid">
        {visDemos.map((d) => {
          ...
          return (
            <button className="card demo-card" key={d.id} onClick={() => setOpenDemo(d)}>
              <div className="demo-card-head">
                <span className="crop-chip lg">{c.icon}</span>
                <div className="demo-card-title"><strong>{c.name} demo</strong><span className="muted-sm"><MapPin size={12} /> {f?.name} · {f?.village}</span></div>
                <span className={"badge " + (done ? "badge-amber" : "badge-green")}>{done ? "Harvest" : c.stages[d.stageIndex]}</span>
              </div>
              <GrowthTrack stages={c.stages} index={d.stageIndex} />
              <div className="demo-card-foot">
                <span className="tiny"><Sprout size={12} /> {byId(d.assignedTo).name}</span>
                <span className="tiny"><CalendarDays size={12} /> {fmtDate(d.start)}</span>
                <span className="tiny">{d.productIds.length} products</span>
              </div>
            </button>
          );
        })}
...
function DemoDetail({ demo, crops, farmers, products, onBack, onAdvance }) {
  const [d, setD] = useState(demo);
  const c = crops.find((x) => x.id === d.cropId);
  const f = farmers.find((x) => x.id === d.farmerId);
  const prods = products.filter((p) => d.productIds.includes(p.id));
  const done = d.stageIndex >= c.stages.length - 1;
  const advance = () => { onAdvance(d.id); setD((s) => ({ ...s, stageIndex: Math.min(s.stageIndex + 1, c.stages.length - 1) })); };
  return (
    <div className="stack">
      <button className="back" onClick={onBack}><ChevronRight size={16} style={{ transform: "rotate(180deg)" }} /> Back</button>
      <div className="detail-hero">
        <span className="crop-emoji xl">{c.icon}</span>
        <div><h2 className="detail-title">{c.name} demo</h2><p className="detail-sub"><MapPin size={13} /> {f?.name} · {f?.village} · {f?.acres} acres · started {fmtDate(d.start)}</p></div>
        <span className={"badge lg " + (done ? "badge-amber" : "badge-green")}>{done ? "Completed" : "In progress"}</span>
      </div>
      <div className="card">
        <div className="card-head"><h3>Growth progress</h3><span className="muted-sm">{d.stageIndex + 1} of {c.stages.length} stages</span></div>
        <div className="track-full-wrap"><GrowthTrack stages={c.stages} index={d.stageIndex} /></div>
        <div className="advance-bar">
          <div><div className="advance-label">Current stage</div><div className="advance-stage"><Sprout size={16} /> {c.stages[d.stageIndex]}</div></div>
          {done ? <span className="done-pill"><Check size={15} /> Demo reached harvest</span>
                : <button className="btn btn-primary" onClick={advance}><Check size={16} /> Complete “{c.stages[d.stageIndex]}” → next</button>}
        </div>
      </div>
      <div className="two-col">
        <div className="card">
          ...
          <div className="farmer-detail">
            <div className="fd-avatar">{initials(f.name)}</div>
            <div>
              <div className="fd-name">{f.name}</div>
              <div className="fd-line"><Phone size={13} /> {f.phone}</div>
              <div className="fd-line"><MapPin size={13} /> {f.village}</div>
              <div className="fd-line"><Ruler size={13} /> {f.acres} acres · {f.lat}, {f.lng}</div>
            </div>
          </div>
        </div>
        <div className="card">
          <div className="card-head"><h3>Products in this demo</h3></div>
          <div className="stack-xs">
            {prods.map((p) => (
              <div className="prod-line" key={p.id}><span className={"badge badge-" + catTone[p.cat]}>{p.cat}</span><span className="pl-name">{p.name}</span><span className="pl-brand">{p.brand}</span><span className="pl-price"><IndianRupee size={12} />{p.price}</span></div>
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="stack">
      ...
      <div className="legend">
        <span><span className="dot-mini done" /> done</span>
        <span><span className="dot-mini cur" /> current</span>
        <span><span className="dot-mini todo" /> upcoming</span>
        <span className="legend-sep" />
        <span>{totalActive} active · {visDemos.length} demos · {rows.length} farmers</span>
      </div>
      ...
      <div className="matrix-scroll">
        <table className="matrix">
          <thead>
            <tr>
              <th className="mx-corner">Farmer / Crop</th>
              {crops.map((c) => <th key={c.id} className="mx-crop-head"><span className="mx-crop-emoji">{c.icon}</span>{c.name}</th>)}
            </tr>
          </thead>
          <tbody>
            {rows.map((f) => (
              <tr key={f.id}>
                <td className="mx-farmer">
                  <button onClick={() => onOpenFarmer(f)}>
                    <span className="fd-avatar xs">{initials(f.name)}</span>
                    <span><span className="mxf-name">{f.name}</span><span className="mxf-village"><MapPin size={10} /> {f.village}</span></span>
                  </button>
                </td>
                {crops.map((c) => {
                  const d = cellDemo(f.id, c.id);
                  if (!d) return <td key={c.id}><button className="cell empty" onClick={() => onStart(f.id, c.id)}><Plus size={14} /></button></td>;
                  const done = d.stageIndex >= c.stages.length - 1;
                  return (
                    <td key={c.id}>
                      <button className={"cell filled" + (done ? " done" : "")} onClick={() => onOpenDemo(d)}>
                        <span className="cell-stage">{done ? "Harvest ✓" : c.stages[d.stageIndex]}</span>
...
function Farmers({ visFarmers, demos, crops, caps, me, setModal, setOpenFarmer }) {
  return (
    <div className="stack">
      <SectionHead
        title={caps.scope === "own" ? "My farmers" : "Farmers"}
        sub={caps.scope === "own" ? "Only farmers you personally registered are visible to you"
          : caps.scope === "team" ? "Farmers registered by your field team" : "Every farmer across the agency"}
      ...
      <div className="farmer-grid">
        {visFarmers.map((f) => {
          const fd = demos.filter((d) => d.farmerId === f.id);
          return (
            <button className="card farmer-card" key={f.id} onClick={() => setOpenFarmer(f)}>
              <div className="fc-head"><div className="fd-avatar">{initials(f.name)}</div>
                <div><div className="fc-name">{f.name}</div><div className="fc-village"><MapPin size={12} /> {f.village}</div></div></div>
              <div className="fc-lines"><span><Phone size={13} /> {f.phone}</span><span><Ruler size={13} /> {f.acres} acres</span></div>
              <div className="fc-crops">
                {fd.length ? fd.map((d) => { const c = crops.find((x) => x.id === d.cropId); return <span key={d.id} className="crop-mini">{c.icon} {c.name} · {c.stages[d.stageIndex]}</span>; })
                           : <span className="crop-mini none">No demos yet</span>}
              </div>
...
function FarmerDetail({ farmer, demos, crops, me, onBack, onOpenDemo, onStart, onAdvance }) {
  ...
  return (
    <div className="stack">
      ...
      <div className="detail-hero">
        <div className="fd-avatar xl">{initials(farmer.name)}</div>
        <div>
          <h2 className="detail-title">{farmer.name}</h2>
          <p className="detail-sub"><MapPin size={13} /> {farmer.village} · <Phone size={13} /> {farmer.phone} · <Ruler size={13} /> {farmer.acres} acres · <LocateFixed size={13} /> {farmer.lat}, {farmer.lng}</p>
        </div>
        <button className="btn btn-primary" onClick={() => onStart(farmer.id, null)}><Plus size={16} /> Start another demo</button>
      </div>

      <div className="card">
        <div className="card-head"><h3>Demos for this farmer</h3><span className="muted-sm">{fd.length} across crops · each on its own stage</span></div>
        <div className="fd-demos">
          {fd.map((d) => {
            const c = crops.find((x) => x.id === d.cropId);
            const done = d.stageIndex >= c.stages.length - 1;
            return (
              <div className="fd-demo" key={d.id}>
                <div className="fd-demo-head">
                  <span className="crop-chip">{c.icon}</span>
                  <div className="fd-demo-title"><strong>{c.name}</strong><span className="muted-sm">started {fmtDate(d.start)}</span></div>
                  <span className={"badge " + (done ? "badge-amber" : "badge-green")}>{done ? "Harvest" : c.stages[d.stageIndex]}</span>
                </div>
                <GrowthTrack stages={c.stages} index={d.stageIndex} />
                <div className="fd-demo-actions">
                  <button className="btn btn-ghost btn-sm" onClick={() => onOpenDemo(d)}>Open demo</button>
                  {!done && <button className="btn btn-primary btn-sm" onClick={() => onAdvance(d.id)}><Check size={14} /> Advance stage</button>}
                </div>
              </div>
            );
          })}
...
function Meetings({ visMeetings, setModal }) {
  ...
  return (
    <div className="stack">
      <SectionHead title="Meetings" sub="Farmer meets, team syncs and product trainings — everyone can schedule"
        action={<button className="btn btn-primary" onClick={() => setModal({ type: "meeting" })}><Plus size={16} /> Schedule meeting</button>} />
      <div className="meet-list">
        {sorted.map((m) => (
          <div className="card meet-card" key={m.id}>
            <div className="meet-date"><span>{fmtDate(m.date).split(" ")[0]}</span><em>{fmtDate(m.date).split(" ")[1]}</em></div>
            <div className="meet-main">
              <div className="meet-title-row"><strong>{m.title}</strong><span className={"badge badge-" + tone[m.type]}>{m.type}</span></div>
              <div className="meet-sub"><Clock size={13} /> {m.time} <span className="sep">·</span> <MapPin size={13} /> {m.place} <span className="sep">·</span> {byId(m.host).name}</div>
            </div>
          </div>
        ))}
      </div>
...
function Team({ me, caps, farmers, demos }) {
  const stat = (empId) => ({ farmers: farmers.filter((f) => f.addedBy === empId).length, demos: demos.filter((d) => d.assignedTo === empId).length });
  const managers = caps.manageTeam ? PEOPLE.filter((p) => p.role === "manager") : PEOPLE.filter((p) => p.id === me.id);
  return (
    <div className="stack">
      <SectionHead title={caps.manageTeam ? "Team" : "Your field team"}
        sub={caps.manageTeam ? "The whole agency — owner, managers and the officers under each" : "The officers reporting to you and what they're running"} />
      {caps.manageTeam && (
        <div className="card owner-card"><div className="fd-avatar owner">{initials("Rajesh Patel")}</div>
          <div><div className="fc-name">Rajesh Patel <span className="badge badge-amber">Owner</span></div><div className="fc-village">Agency Owner · Saurashtra</div></div></div>
      )}
      {managers.map((mgr) => {
        const emps = PEOPLE.filter((p) => p.manager === mgr.id);
        return (
          <div className="card mgr-block" key={mgr.id}>
            <div className="mgr-head"><div className="fd-avatar mgr">{initials(mgr.name)}</div>
              <div><div className="fc-name">{mgr.name} <span className="badge badge-sky">{caps.manageTeam ? "Manager" : "You"}</span></div><div className="fc-village">{mgr.title}</div></div>
              <span className="mgr-count">{emps.length} field officers</span></div>
            <div className="emp-row">
              {emps.map((e) => { const s = stat(e.id); return (
                <div className="emp-card" key={e.id}><div className="fd-avatar sm">{initials(e.name)}</div><div className="emp-name">{e.name}</div>
                  <div className="emp-stats"><span>{s.farmers} farmers</span><span>{s.demos} demos</span></div></div>
              ); })}
            </div>
          </div>
        );
...
/* ============================================================ shared */
function SectionHead({ title, sub, action, note }) {
  return (<div className="section-head"><div><h2 className="sh-title">{title}</h2><p className="sh-sub">{sub}</p>{note}</div>{action}</div>);
}
...
/* geolocation capture block, shared by farmer add + field demo */
function GeoCapture({ loc, setLoc, onVillage }) {
  const fetchLoc = () => {
    setLoc({ status: "loading" });
    const fallback = () => {
      const v = NEARBY_VILLAGES[Math.floor(Math.random() * NEARBY_VILLAGES.length)];
      const lat = (21.6 + Math.random() * 0.9).toFixed(5), lng = (70.6 + Math.random() * 0.9).toFixed(5);
      setLoc({ status: "done", lat, lng, village: v, approx: true }); onVillage && onVillage(v);
    };
    if (navigator.geolocation) {
      const t = setTimeout(fallback, 6500);
      navigator.geolocation.getCurrentPosition(
        (pos) => { clearTimeout(t); setLoc({ status: "done", lat: pos.coords.latitude.toFixed(5), lng: pos.coords.longitude.toFixed(5), village: "", approx: false }); },
        () => { clearTimeout(t); fallback(); }, { enableHighAccuracy: true, timeout: 6000 });
    } else fallback();
  };
  return (
...
function ProductModal({ onClose, onSave }) {
  ...
  return (
    <Modal title="Add product" subtitle="New entry in the product master" onClose={onClose}
      ...
      <div className="grid2">
        <Field label="Category"><select value={f.cat} onChange={set("cat")}><option>Seed</option><option>Fertilizer</option><option>Pesticide</option></select></Field>
        <Field label="Pack size"><input value={f.pack} onChange={set("pack")} placeholder="e.g. 450 g" /></Field>
      </div>
...
function CropModal({ onClose, onSave }) {
  ...
  const [stages, setStages] = useState(["Sowing", "Germination", "Harvest"]);
  const [stg, setStg] = useState("");
  const set = (k) => (e) => setF((s) => ({ ...s, [k]: e.target.value }));
  const addStg = () => { if (stg.trim()) { setStages((s) => [...s, stg.trim()]); setStg(""); } };
  const ok = f.name && stages.length >= 2;
  return (
    <Modal title="New crop" subtitle="Define a crop and the stages its demos will move through" onClose={onClose}
      ...
      <div className="grid2">
        <Field label="Crop name"><input value={f.name} onChange={set("name")} placeholder="e.g. Castor" /></Field>
        <Field label="Season"><select value={f.season} onChange={set("season")}><option>Kharif</option><option>Rabi</option><option>Zaid</option></select></Field>
      </div>
      <Field label="Icon">
        <div className="emoji-pick">{EMOJIS.map((e) => <button key={e} type="button" className={"emoji-opt" + (f.icon === e ? " on" : "")} onClick={() => setF((s) => ({ ...s, icon: e }))}>{e}</button>)}</div>
      </Field>
      <Field label="Stage pipeline (in order)">
        <div className="stage-edit">
          {stages.map((s, i) => (
            <span className="stage-tag" key={i}>{i + 1}. {s}<button type="button" onClick={() => setStages((x) => x.filter((_, j) => j !== i))}><X size={11} /></button></span>
          ))}
        </div>
        <div className="stage-add">
          <input value={stg} onChange={(e) => setStg(e.target.value)} placeholder="Add a stage…" onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addStg())} />
          <button type="button" className="btn btn-ghost btn-sm" onClick={addStg}><Plus size={14} /> Add</button>
        </div>
      </Field>
    </Modal>
  );
...
function FarmerModal({ onClose, onSave }) {
  const [f, setF] = useState({ name: "", phone: "", village: "", acres: "" });
  ...
  return (
    <Modal title="Add farmer" subtitle="Capture live location on the field — a farmer is registered only once" onClose={onClose}
      ...
      <div className="grid2">
        <Field label="Phone"><input value={f.phone} onChange={set("phone")} placeholder="98240 11223" /></Field>
        <Field label="Land (acres)"><input type="number" value={f.acres} onChange={set("acres")} placeholder="6" /></Field>
      </div>
      <Field label="Village"><input value={f.village} onChange={set("village")} placeholder="Village name" /></Field>
    </Modal>
  );
}
function DemoModal({ onClose, onSave, crops, products, visFarmers, presetFarmerId, presetCropId }) {
  ...
  const [d, setD] = useState({
    cropId: presetCropId || crops[0].id,
    farmerId: presetFarmerId || visFarmers[0]?.id || "",
    productIds: [], start: "2026-08-21",
  });
  const [nf, setNf] = useState({ name: "", phone: "", village: "", acres: "" });
  const [loc, setLoc] = useState(null);
  const setNfk = (k) => (e) => setNf((s) => ({ ...s, [k]: e.target.value }));
  const toggleProd = (id) => setD((s) => ({ ...s, productIds: s.productIds.includes(id) ? s.productIds.filter((x) => x !== id) : [...s.productIds, id] }));

  const newOk = nf.name && nf.phone && nf.village && nf.acres && loc?.status === "done";
  const ok = d.cropId && d.productIds.length > 0 && (mode === "existing" ? d.farmerId : newOk);

  const submit = () => onSave(mode === "existing"
    ? { mode: "existing", farmerId: d.farmerId, cropId: d.cropId, productIds: d.productIds, start: d.start }
    : { mode: "new", farmer: { ...nf, acres: Number(nf.acres), lat: loc.lat, lng: loc.lng }, cropId: d.cropId, productIds: d.productIds, start: d.start });

  return (
    <Modal title="Start crop demo" subtitle="On the field: pick a farmer or add a new one on the spot, then the crop" onClose={onClose}
      footer={<><button className="btn btn-ghost" onClick={onClose}>Cancel</button><button className="btn btn-primary" disabled={!ok} onClick={submit}>Start demo</button></>}>
      {!lockedFarmer && (
        <div className="seg">
          <button className={mode === "existing" ? "on" : ""} onClick={() => setMode("existing")} disabled={!visFarmers.length}>Existing farmer</button>
          <button className={mode === "new" ? "on" : ""} onClick={() => setMode("new")}>New farmer (on the ride)</button>
        </div>
      )}

      {mode === "existing"
        ? (visFarmers.length === 0
            ? <div className="warn"><Users size={16} /> No farmers yet — switch to “New farmer” to add one from the field.</div>
            : lockedFarmer
              ? <div className="locked-farmer"><div className="fd-avatar sm">{initials(lockedFarmer.name)}</div><div><strong>{lockedFarmer.name}</strong><span className="muted-sm">{lockedFarmer.village}</span></div></div>
        ...
        : <>
            ...
            <div className="grid2">
              <Field label="Phone"><input value={nf.phone} onChange={setNfk("phone")} placeholder="98xxx xxxxx" /></Field>
              <Field label="Land (acres)"><input type="number" value={nf.acres} onChange={setNfk("acres")} placeholder="5" /></Field>
            </div>
      ...
      <Field label="Products to demonstrate">
        <div className="prod-pick">{products.map((p) => <button key={p.id} type="button" className={"pick" + (d.productIds.includes(p.id) ? " on" : "")} onClick={() => toggleProd(p.id)}>{d.productIds.includes(p.id) && <Check size={12} />} {p.name}</button>)}</div>
      </Field>
    </Modal>
  );
}
function MeetingModal({ onClose, onSave }) {
  const [f, setF] = useState({ title: "", type: "Farmer Meet", date: "2026-08-25", time: "10:00", place: "" });
  const set = (k) => (e) => setF((s) => ({ ...s, [k]: e.target.value }));
...
.gg{
  ...
  --primary:#1F6B3B; --primary-d:#17542E; --accent:#7CB342; --amber:#C67A1E; --sky:#2F7FA3; --violet:#7C5CBF;
...
.gg button{font-family:inherit; cursor:pointer;} .gg input,.gg select{font-family:inherit;}
...
.nav-item.active{background:linear-gradient(90deg,rgba(31,107,59,.12),rgba(124,179,66,.06)); color:var(--primary); font-weight:600;}
...
.page-head h1{font-size:18px;} .page-head p{font-size:12px; color:var(--ink-soft); margin-top:1px;}
.role-pill{text-transform:capitalize; color:var(--primary); font-weight:600;}
...
.dot{position:absolute; top:8px; right:9px; width:7px; height:7px; border-radius:50%; background:var(--amber); border:2px solid var(--surface);}
...
.hero-title{font-family:var(--display); font-size:26px; line-height:1.15; font-weight:700; max-width:560px;}
.hero-sub{color:#d4ead9; font-size:13.5px; margin-top:8px;}
...
.kpi-num{font-family:var(--display); font-size:30px; font-weight:700; line-height:1;}
.kpi-label{font-size:13px; font-weight:600; margin-top:6px;} .kpi-sub{font-size:11.5px; color:var(--ink-soft);}

.card{background:var(--surface); border:1px solid var(--line); border-radius:16px; padding:18px; box-shadow:var(--shadow);}
.card-head{display:flex; align-items:center; justify-content:space-between; margin-bottom:14px;} .card-head.mt{margin-top:22px;} .card-head h3{font-size:15px;}
...
.demo-row-main{flex:1; min-width:0;} .demo-row-top{display:flex; align-items:center; gap:9px; margin-bottom:11px;} .demo-row-top strong{font-size:13.5px;}
...
.node-label{position:absolute; top:26px; font-size:10.5px; color:var(--ink-soft); white-space:nowrap; font-weight:500;}
.node.current .node-label{color:var(--primary); font-weight:700;} .node.done .node-label{color:var(--ink);}
...
.dot-mini.done{background:var(--primary);} .dot-mini.cur{background:var(--accent); box-shadow:0 0 0 3px rgba(124,179,66,.2);} .dot-mini.todo{background:var(--line);}

.bars{display:flex; flex-direction:column; gap:12px;}
.bar-row{display:grid; grid-template-columns:96px 1fr 24px; align-items:center; gap:10px;}
.bar-label{font-size:12.5px; font-weight:500;}
...
.bar-val{font-weight:700; font-size:13px; text-align:right; font-family:var(--display);}
...
.badge-green{background:rgba(31,107,59,.12); color:var(--primary);} .badge-amber{background:rgba(198,122,30,.14); color:var(--amber);}
.badge-sky{background:rgba(47,127,163,.13); color:var(--sky);} .badge-muted{background:var(--surface-2); color:var(--ink-soft);}

.section-head{display:flex; align-items:flex-start; justify-content:space-between; gap:16px;}
.sh-title{font-size:21px;} .sh-sub{color:var(--ink-soft); font-size:13.5px; margin-top:4px; max-width:600px;}
...
.btn-primary{background:var(--primary); color:#fff; box-shadow:0 3px 10px rgba(31,107,59,.28);}
.btn-primary:hover{background:var(--primary-d); transform:translateY(-1px);} .btn-primary:disabled{opacity:.45; cursor:not-allowed; transform:none;}
...
.btn-sky{background:var(--sky); color:#fff;} .btn-sky:hover{filter:brightness(.94);}
...
.prod-card{background:var(--surface); border:1px solid var(--line); border-radius:14px; padding:15px; box-shadow:var(--shadow); transition:.15s;}
...
.crop-card{text-align:left; background:var(--surface); border:1px solid var(--line); border-radius:16px; padding:18px; box-shadow:var(--shadow); display:flex; flex-direction:column; gap:6px; transition:.15s;}
.crop-card:hover{transform:translateY(-2px); border-color:var(--accent);}
...
.pipeline{display:flex; flex-wrap:wrap; gap:6px 4px; align-items:center;}
.pipe-step{display:flex; align-items:center; gap:8px;}
.pipe-num{font-family:var(--display); font-weight:700; font-size:12px; color:var(--accent); background:rgba(124,179,66,.12); width:24px; height:24px; border-radius:7px; display:grid; place-items:center;}
.pipe-name{font-weight:600; font-size:13px;} .pipe-arrow{color:var(--line); margin:0 8px;}
.add-stage{display:flex; gap:10px; margin-top:18px; padding-top:16px; border-top:1px solid var(--line);}
.add-stage input{flex:1; padding:10px 13px; border:1px solid var(--line); border-radius:10px; outline:none; font-size:13.5px;} .add-stage input:focus{border-color:var(--accent);}
...
.demo-card-foot{display:flex; gap:14px; padding-top:12px; margin-top:2px; border-top:1px solid var(--line); flex-wrap:wrap;}
.tiny{font-size:11.5px; color:var(--ink-soft); display:inline-flex; align-items:center; gap:5px;}

.advance-bar{display:flex; align-items:center; justify-content:space-between; gap:14px; margin-top:18px; padding:16px; background:var(--surface-2); border-radius:13px; border:1px solid var(--line);}
...
.prod-line{display:flex; align-items:center; gap:10px; padding:10px; background:var(--surface-2); border-radius:10px;}
.pl-name{font-weight:600; font-size:13px;} .pl-brand{font-size:11.5px; color:var(--ink-soft);} .pl-price{margin-left:auto; font-family:var(--display); font-weight:700; display:inline-flex; align-items:center;}
...
.fd-demo-actions{display:flex; gap:8px; justify-content:flex-end; margin-top:4px;}
...
.legend{display:flex; align-items:center; gap:16px; font-size:12px; color:var(--ink-soft); flex-wrap:wrap;}
.legend span{display:inline-flex; align-items:center; gap:6px;} .legend-sep{width:1px; height:14px; background:var(--line);}
...
.matrix th,.matrix td{border-bottom:1px solid var(--line); border-right:1px solid var(--line); padding:0;}
.matrix th:last-child,.matrix td:last-child{border-right:none;} .matrix tr:last-child td{border-bottom:none;}
...
.mx-crop-head{background:var(--surface-2); padding:12px 14px; font-family:var(--font); font-size:13px; font-weight:700; white-space:nowrap;}
.mx-crop-emoji{margin-right:6px;}
.mx-farmer{position:sticky; left:0; background:var(--surface); z-index:1;}
.mx-farmer button{display:flex; align-items:center; gap:10px; padding:11px 14px; width:100%; background:none; border:none; text-align:left; transition:.15s;}
.mx-farmer button:hover{background:var(--surface-2);}
...
.cell{display:flex; flex-direction:column; align-items:flex-start; gap:7px; width:100%; padding:12px 14px; background:none; border:none; transition:.15s; min-height:60px;}
.cell.filled{cursor:pointer;} .cell.filled:hover{background:rgba(124,179,66,.08);}
.cell.filled.done{background:rgba(198,122,30,.06);}
.cell-stage{font-size:12px; font-weight:600; color:var(--ink);} .cell.done .cell-stage{color:var(--amber);}
.cell.empty{align-items:center; justify-content:center; color:var(--line); min-height:60px;} .cell.empty:hover{background:var(--surface-2); color:var(--accent);}
...
.mgr-head{display:flex; align-items:center; gap:13px; padding:16px 18px; background:var(--surface-2); border-bottom:1px solid var(--line);}
.mgr-count{margin-left:auto; font-size:12px; color:var(--ink-soft); font-weight:600;}
...
.emp-name{font-weight:600; font-size:13.5px;} .emp-stats{display:flex; gap:8px; font-size:11px; color:var(--ink-soft);}
.emp-stats span{background:var(--surface); border:1px solid var(--line); padding:3px 8px; border-radius:7px;}
...
.overlay{position:fixed; inset:0; background:rgba(19,32,24,.5); backdrop-filter:blur(3px); z-index:100; display:flex; align-items:flex-start; justify-content:center; padding:5vh 16px; overflow:auto; animation:fade .2s;}
@keyframes fade{from{opacity:0;}}
.modal{background:var(--surface); border-radius:18px; width:100%; max-width:460px; box-shadow:0 20px 60px rgba(19,32,24,.3); animation:rise .25s;}
@keyframes rise{from{transform:translateY(14px); opacity:0;}}
.modal-head{display:flex; align-items:flex-start; justify-content:space-between; padding:20px 20px 6px;}
.modal-head h3{font-size:18px;} .modal-head p{font-size:12.5px; color:var(--ink-soft); margin-top:3px; max-width:360px;}
...
.field input,.field select{padding:10px 13px; border:1px solid var(--line); border-radius:10px; outline:none; font-size:13.5px; background:var(--surface); color:var(--ink);}
.field input:focus,.field select:focus{border-color:var(--accent); box-shadow:0 0 0 3px rgba(124,179,66,.14);}
.grid2{display:grid; grid-template-columns:1fr 1fr; gap:12px;}

.seg{display:flex; background:var(--surface-2); border:1px solid var(--line); border-radius:11px; padding:3px; gap:2px;}
.seg button{flex:1; padding:9px; border:none; border-radius:8px; background:none; font-size:12.5px; font-weight:600; color:var(--ink-soft); transition:.15s;}
.seg button.on{background:var(--primary); color:#fff;} .seg button:disabled{opacity:.4; cursor:not-allowed;}
.locked-farmer{display:flex; align-items:center; gap:11px; padding:11px 13px; background:var(--surface-2); border:1px solid var(--line); border-radius:11px;}
.locked-farmer strong{display:block; font-size:13.5px;} .locked-farmer .muted-sm{margin-top:1px;}
...
.emoji-opt{width:40px; height:40px; border-radius:10px; border:1px solid var(--line); background:var(--surface); font-size:19px; transition:.15s;}
.emoji-opt:hover{border-color:var(--accent);} .emoji-opt.on{border-color:var(--primary); background:rgba(31,107,59,.08); box-shadow:0 0 0 2px rgba(31,107,59,.15);}
.stage-edit{display:flex; flex-wrap:wrap; gap:6px; margin-bottom:8px;}
.stage-tag{display:inline-flex; align-items:center; gap:6px; font-size:11.5px; font-weight:600; padding:4px 6px 4px 10px; border-radius:8px; background:rgba(31,107,59,.08); color:var(--primary);}
.stage-tag button{background:none; border:none; color:var(--primary); display:grid; place-items:center; opacity:.6;} .stage-tag button:hover{opacity:1;}
.stage-add{display:flex; gap:8px;} .stage-add input{flex:1; padding:9px 12px; border:1px solid var(--line); border-radius:9px; outline:none; font-size:13px;} .stage-add input:focus{border-color:var(--accent);}

.prod-pick{display:flex; flex-wrap:wrap; gap:7px;}
.pick{display:inline-flex; align-items:center; gap:5px; padding:7px 11px; border-radius:9px; border:1px solid var(--line); background:var(--surface); font-size:12px; font-weight:500; color:var(--ink-soft); transition:.15s;}
.pick:hover{border-color:var(--accent);} .pick.on{background:var(--primary); color:#fff; border-color:var(--primary); font-weight:600;}
...
.toast{position:fixed; bottom:26px; left:50%; transform:translateX(-50%); z-index:200; display:flex; align-items:center; gap:9px; background:var(--ink); color:#fff; padding:12px 18px; border-radius:12px; font-size:13.5px; font-weight:500; box-shadow:0 12px 34px rgba(19,32,24,.35); animation:toast .3s;}
.toast svg{color:var(--accent);} @keyframes toast{from{transform:translate(-50%,14px); opacity:0;}}

.only-mobile{display:none;} .drawer-x{display:none;} .scrim{display:none;}
@media(max-width:1000px){ .two-col{grid-template-columns:1fr;} .kpi-grid{grid-template-columns:repeat(2,1fr);} }
@media(max-width:820px){
  ...
  .section-head{flex-direction:column;} .detail-hero{flex-wrap:wrap;} .detail-hero .btn-primary{margin-left:0;}
}
@media(max-width:520px){ .kpi-grid{grid-template-columns:repeat(2,1fr);} .roleswitch .rs-opt span{display:none;} .grid2{grid-template-columns:1fr;} .node-label{font-size:9px;} }
